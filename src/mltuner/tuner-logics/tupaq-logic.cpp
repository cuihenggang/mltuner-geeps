...
#include "mltuner/tuner-logics/tupaq-logic.hpp"
...
#define TUPAQ_CHECK_EPOCH  10


TupaqLogic::TupaqLogic(MltunerImpl *impl, const Config& config)
    : TunerLogic(impl, config) {
}

void TupaqLogic::make_branch_decisions() {
...
void TupaqLogic::make_and_schedule_initial_branches() {
...
void TupaqLogic::state_machine__start_searching() {
  best_accuracy_ = std::numeric_limits<double>::quiet_NaN();
...
void TupaqLogic::state_machine__running() {
...
void TupaqLogic::state_machine__testing() {
  /* Finish testing */
  ...
  exp_info.peak_accuracy = peak_accuracy;
  if (converged) {
    cout << "Stopping condition met, accuracy = " << peak_accuracy
         << endl << endl;
  }
  if (exp_info.epoch >= TUPAQ_CHECK_EPOCH &&
      !isnan(best_accuracy_) && peak_accuracy < best_accuracy_ * 0.5) {
    converged = true;
    cout << "Stop early because of bandit, accuracy = " << peak_accuracy
         << endl << endl;
  }
  ...
  if (!converged) {
    ...
    if (isnan(best_accuracy_) || best_accuracy_ < peak_accuracy) {
      best_accuracy_ = peak_accuracy;
    }
...
bool TupaqLogic::try_new_setting() {

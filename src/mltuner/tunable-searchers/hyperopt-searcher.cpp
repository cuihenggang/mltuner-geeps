...
HyperoptSearcher::HyperoptSearcher(
    const Config& config,
...
bool HyperoptSearcher::get_result(
...
void HyperoptSearcher::set_result(uint exp_id, double cost, double loss) {
  ScopedLock lock(mutex_);
...
void HyperoptSearcher::start() {
  /* Start searcher process */
  background_thread_ = make_shared<boost::thread>(
      boost::bind(&HyperoptSearcher::background_thread_entry, this));
}

void HyperoptSearcher::stop() {
...
void HyperoptSearcher::operator()() {
  background_thread_entry();
}

void HyperoptSearcher::background_thread_entry() {
  searcher_cmd_ =
      (boost::format("python %s/call_hyperopt_%s.py %s")
...
void HyperoptSearcher::call_searcher() {
...
void HyperoptSearcher::write_result_file() {
...
void HyperoptSearcher::read_result_file() {
  while (true) {
...
bool HyperoptSearcher::check_convergence() {
  DoubleVec perfs;
  for (uint i = 0; i < tunable_exp_results_.size(); i++) {
...
bool HyperoptSearcher::is_topk_close_enough(
...
bool HyperoptSearcher::is_no_improvement_over_lastk(

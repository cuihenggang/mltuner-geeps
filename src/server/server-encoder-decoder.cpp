...
void ClientServerDecode::clock(
  ...
  int branch_id = cs_clock_msg->branch_id;
  iter_t internal_clock = cs_clock_msg->internal_clock;

  storage->clock(client_id, clock, table_id, branch_id, internal_clock);
...
void ClientServerDecode::clock_with_updates_batch(
  ...
  int branch_id = cs_clock_with_updates_batch_msg->branch_id;
  iter_t internal_clock = cs_clock_with_updates_batch_msg->internal_clock;
  ...
  storage->update_row_batch(
      client_id, clock, table_id, branch_id,
      row_keys, updates, batch_size);
...
void ClientServerDecode::worker_started(
    const string& src, vector<ZmqPortableBytes>& args) {
  CHECK_EQ(args.size(), 1);
...
void ClientServerDecode::make_branch(
    const string& src, vector<ZmqPortableBytes>& args) {
  CHECK_EQ(args.size(), 2);
  ...
  uint branch_id = make_branch_msg->branch_id;
  int flag = make_branch_msg->flag;
  iter_t parent_branch_id = make_branch_msg->parent_branch_id;
  int clock_to_make = make_branch_msg->clock_to_make;

  Tunable& tunable =
      *(reinterpret_cast<Tunable *>(args[1].data()));

  storage->make_branch(
      branch_id, tunable, flag, parent_branch_id, clock_to_make);
...
void ClientServerDecode::inactivate_branch(
    const string& src, vector<ZmqPortableBytes>& args) {
  CHECK_EQ(args.size(), 1);
  ...
  uint branch_id = inactivate_branch_msg->branch_id;
  int clock_to_happen = inactivate_branch_msg->clock_to_happen;

  storage->inactivate_branch(branch_id, clock_to_happen);
...
void ClientServerDecode::decode_msg(
  ...
  // cout << "server of channel " << channel_id << " receives message\n";
  ...
  case CLOCK_WITH_UPDATES_BATCH:
    // cout << "server receives CLOCK_WITH_UPDATES_BATCH\n";
  ...
  case WORKER_STARTED:
    ...
    break;
  case MAKE_BRANCH:
    // cout << "server receives FORWARD_MAKE_BRANCH\n";
    make_branch(src, msgs);
    break;
  case INACTIVATE_BRANCH:
    inactivate_branch(src, msgs);
    break;
  default:
    CHECK(0)
...
void ServerClientEncode::read_row_batch_reply(
  ...
  sc_read_row_batch_msg->branch_id = branch_id;
  sc_read_row_batch_msg->internal_data_age = internal_data_age;
  sc_read_row_batch_msg->internal_self_clock = internal_self_clock;

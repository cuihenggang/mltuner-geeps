...
def get_conv(
    ...
    return (
        ...
        mxc.Activation(name=name + '_relu', data=bn, act_type='relu')
...
def make_block(
    ...
    fused = mxc.Add(
        name=name + '_fused',
        bottoms=[shortcut,conv2]
    )
    return mxc.Activation(
        name=name + '_relu',
        data=fused,
        act_type='relu'
    )
...
def get_symbol(
    ...
    pool = mxc.Pooling(data=body, name='pool', kernel=pool_kernel, pool_type='avg')

...
pdsh -R ssh -w ^examples/cifar10/2parts/machinefile "cd $(pwd) && ./build/tools/caffe_geeps train --solver=examples/cifar10/2parts/inception_solver.prototxt --ps_config=examples/cifar10/2parts/ps_config_inception --machinefile=examples/cifar10/2parts/machinefile --output_dir=$1 --worker_id=%n" 2>&1 | tee ${LOG}

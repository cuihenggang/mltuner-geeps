...
pdsh -R ssh -w ^$EXP_PATH/machinefile "pkill -9 caffe_geeps"
pdsh -R ssh -w ^$EXP_PATH/machinefile "pkill -9 python"
...
pdsh -R ssh -w ^$EXP_PATH/machinefile "cd $(pwd) && HDF5_DISABLE_VERSION_CHECK=1 PYTHONPATH=$EXP_PATH ./build/tools/caffe_geeps train --solver=$EXP_PATH/lstm_solver_RGB_googlenet.prototxt --ps_config=$EXP_PATH/ps_config_googlenet --weights=/datasets/BigLearning/hengganc/archive/snapshots_googlenet_singleFrame_RGB_iter_5000.caffemodel --machinefile=$EXP_PATH/machinefile --output_dir=$1 --worker_id=%n" 2>&1 | tee $1/output.txt
